...
class GaussianDiffusion:
    ...
    def p_sample_loop(
        ...
        :param return_all_pred_xstart: if True, return all predicted xstart.
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            if return_all_pred_xstart:
                all_pred_xstart.append(final["pred_xstart"])
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            mask = model_kwargs["y"]["valid_frames"]
            mask = mask.view(list(mask.shape) + [1] * (x_t.ndim - mask.ndim))
            mask = mask.expand_as(target)
            loss_here = sum_flat(((target - model_output) * mask) ** 2)
            denom = sum_flat(mask)
            terms["mse"] = loss_here / denom
