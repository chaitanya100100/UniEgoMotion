...
def _get_joints_to_guofeats():
    ...
    def transform(source_data):
        ...
        if isinstance(source_data, torch.Tensor):
            ...
            return torch.from_numpy(source_data).float()
        # -----------------------------
        ...
        # -----------------------------
        # Added by me
        # For input source_data, we assume same coordinate system as AMASS.
        # It has +Z up, -Y forward and +X left of the person. I think only +Z up matters though.
        # But Guo et. al. did this transformation before computing the features.
        # guofeats_to_joints already does the inverse transformation of this.
        # So I didn't need to add it there.
        # -----------------------------
...
def _get_guofeats_to_joints():
    ...
    def transform(data):
        ...
        # Added by me
        # -----------------------------
        # Repeat the last frame to make it the same length as the original motion
        # Note that the overall rotation of the motion is lost. Reconstructed motion
        # will always start from the origin, looking at -Y direction (after the final transformation of course).
        joints = torch.cat([joints, joints[..., -1:, :, :]], dim=-3)
        # -----------------------------
